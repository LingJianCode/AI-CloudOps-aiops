...
class ResourceConstraints(BaseModel):
    ...
    max_instances: Optional[int] = Field(
        None, description="最大实例数限制", gt=0, le=1000
    )
    min_instances: Optional[int] = Field(
        None, description="最小实例数限制", gt=0, le=100
    )
...
class BasePredictionRequest(BaseModel):
    ...
    metric_query: Optional[str] = Field(
        None, description="自定义Prometheus指标查询，为空则使用默认指标"
    )
    prediction_hours: int = Field(default=24, description="预测小时数", ge=1, le=168)
    granularity: PredictionGranularity = Field(
        default=PredictionGranularity.HOUR, description="预测时间粒度"
    )
    resource_constraints: Optional[ResourceConstraints] = Field(
        None, description="资源约束配置"
    )
    include_confidence: bool = Field(default=True, description="是否包含置信区间")
    include_anomaly_detection: bool = Field(
        default=True, description="是否包含异常检测"
    )
    consider_historical_pattern: bool = Field(
        default=True, description="是否考虑历史模式"
    )
    target_utilization: float = Field(
        default=0.7, description="目标资源利用率", gt=0, le=1
    )
    sensitivity: float = Field(
        default=0.8, description="异常检测敏感度", ge=0.1, le=1.0
    )
...
class QpsPredictionRequest(BasePredictionRequest):
    ...
    current_qps: float = Field(..., description="当前QPS值", gt=0)

    @validator("current_qps")
    def validate_current_qps(cls, v):
        if v <= 0 or v > 100000:
...
class CpuPredictionRequest(BasePredictionRequest):
    ...
    current_cpu_percent: float = Field(
        ..., description="当前CPU使用率(%)", ge=0, le=100
    )

    @validator("current_cpu_percent")
    def validate_current_cpu_percent(cls, v):
        if v < 0 or v > 100:
            raise ValueError("CPU利用率应在0-100%之间")
        return v
...
class MemoryPredictionRequest(BasePredictionRequest):
    ...
    current_memory_percent: float = Field(
        ..., description="当前内存使用率(%)", ge=0, le=100
    )

    @validator("current_memory_percent")
    def validate_current_memory_percent(cls, v):
        if v < 0 or v > 100:
            raise ValueError("内存利用率应在0-100%之间")
        return v
...
class DiskPredictionRequest(BasePredictionRequest):
    ...
    current_disk_percent: float = Field(
        ..., description="当前磁盘使用率(%)", ge=0, le=100
    )

    @validator("current_disk_percent")
    def validate_current_disk_percent(cls, v):
        if v < 0 or v > 100:
            raise ValueError("磁盘利用率应在0-100%之间")
        return v
...
class PredictionDataPoint(BaseModel):
    ...
    confidence_level: Optional[float] = Field(
        None, description="预测置信度", ge=0, le=1
    )
...
class ScalingRecommendation(BaseModel):
    ...
    target_memory_gb: Optional[float] = Field(
        None, description="目标内存大小(GB)", gt=0
    )
...
class CostAnalysis(BaseModel):
    ...
    current_hourly_cost: Optional[float] = Field(
        None, description="当前每小时成本", ge=0
    )
    predicted_hourly_cost: Optional[float] = Field(
        None, description="预测每小时成本", ge=0
    )
    cost_savings_potential: Optional[float] = Field(None, description="潜在节省成本(%)")
    cost_trend_analysis: Dict[str, Any] = Field(
        default_factory=dict, description="成本趋势分析"
    )
...
class AnomalyPrediction(BaseModel):
    ...
    impact_level: str = Field(
        ..., description="影响等级", pattern="^(low|medium|high|critical)$"
    )
