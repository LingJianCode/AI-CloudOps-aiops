    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "values": [
                    {"timestamp": "2024-01-01T00:00:00Z", "value": 1024000}
                ],
...
class RCAAnalyzeRequest(BaseModel):
    ...
    time_window_hours: float = Field(1.0, ge=0.1, le=24, description="分析时间窗口（小时）")
    incident_time: Optional[datetime] = Field(None, description="事件发生时间")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "incident_time": "2024-01-01T00:00:00Z"
            }
        }
...
# 注意：响应模型已统一使用ResponseWrapper.success()格式，不再需要单独的响应模型


# 注意：已删除未使用的辅助类型 TimeRange, ResourceMetrics, AnalysisContext