...
def root_cause_analysis():
    ...
    try:
        ...
        logger.info(
            f"执行根因分析: {rca_request.start_time} - {rca_request.end_time}, 指标数: {len(rca_request.metrics)}"
        )
        ...
        try:
            result = loop.run_until_complete(
                rca_analyzer.analyze(
                    rca_request.start_time, rca_request.end_time, rca_request.metrics
                )
            )
        finally:
            loop.close()
        ...
        if "error" not in result:
            ...
            return (
                jsonify(
                    APIResponse(
                        code=500, message=f"根因分析失败: {result['error']}", data={}
                    ).dict()
                ),
                500,
            )
        ...
        return (
            jsonify(APIResponse(code=500, message=f"处理请求失败: {str(e)}", data={}).dict()),
            500,
        )
...
def analyze_incident():
    ...
    try:
        ...
        if not affected_services:
            return (
                jsonify(APIResponse(code=400, message="必须指定受影响的服务", data={}).dict()),
                400,
            )
        ...
        try:
            ...
            return (
                jsonify(APIResponse(code=400, message=f"时间格式错误: {str(e)}", data={}).dict()),
                400,
            )
        ...
        return (
            jsonify(APIResponse(code=500, message=f"事件分析失败: {str(e)}", data={}).dict()),
            500,
        )
...
def get_available_metrics():
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if available_metrics:
                    metrics_info["available_from_prometheus"] = available_metrics[
                        :50
                    ]  # 限制返回数量
            finally:
                loop.close()
        ...
        return (
            jsonify(APIResponse(code=500, message=f"获取指标列表失败: {str(e)}", data={}).dict()),
            500,
        )
...
def rca_health():
    ...
    try:
        ...
        health_status = {
            ...
            "healthy": prometheus_healthy and llm_healthy,
            "components": {
...
@rca_bp.route("/rca/ready", methods=["GET"])
def rca_ready():
    """根因分析服务就绪性检查"""
    try:
        # 检查核心组件是否就绪
        prometheus_ready = rca_analyzer.prometheus.is_healthy()
            ...
            return jsonify(
                APIResponse(
                    ...
                    data={
                        "status": "ready",
                        "timestamp": datetime.utcnow().isoformat(),
                        "prometheus_healthy": prometheus_ready,
                    },
                ).dict()
            )
        else:
        ...
        logger.error(f"RCA就绪性检查失败: {str(e)}")
        return (
            jsonify(
                APIResponse(
                    code=500,
...
def get_rca_config():
    ...
    try:
        ...
        return (
            jsonify(APIResponse(code=500, message=f"获取配置失败: {str(e)}", data={}).dict()),
            500,
        )


@rca_bp.route("/rca/info", methods=["GET"])
def rca_info():
    """获取根因分析服务信息"""
    try:
        rca_info = {
            ...
            "components": {
                "prometheus": {
                    "available": rca_analyzer.prometheus.is_healthy(),
                    "base_url": rca_analyzer.prometheus.base_url,
                },
                "llm": {
                    "available": rca_analyzer.llm.is_healthy(),
                    "model": getattr(rca_analyzer.llm, 'model', 'unknown'),
                },
                "detector": {
                    "anomaly_threshold": rca_analyzer.detector.anomaly_threshold,
                    "methods": ["zscore", "iqr", "isolation_forest", "dbscan", "moving_average"],
        ...
        return jsonify(APIResponse(code=0, message="获取RCA服务信息成功", data=rca_info).dict())

    except Exception as e:
        logger.error(f"获取RCA服务信息失败: {str(e)}")
        return (
            jsonify(APIResponse(code=500, message=f"获取服务信息失败: {str(e)}", data={}).dict()),
            500,
        )
