...
class MCPSessionClient:
  ...
  async def _make_get_request(self, endpoint: str, operation_name: str) -> Dict[str, Any]:
    """通用GET请求处理"""
    try:
      async with aiohttp.ClientSession() as session:
        async with session.get(f"{self.server_url}{endpoint}") as response:
          if response.status == 200:
            ...
            raise RuntimeError(f"{operation_name}失败，状态码: {response.status}")
    except aiohttp.ClientError as e:
      logger.error(f"MCP客户端网络错误: {str(e)}")
      raise RuntimeError(f"MCP服务连接失败: {str(e)}")
    except Exception as e:
...
class MCPAssistant:
  ...
  async def process_query(self, question: str) -> str:
    ...
    try:
      ...
      # 构建系统提示
      system_content = """你是一个智能助手，能够根据用户的问题自主选择合适的工具来回答。请分析用户的问题，判断是否需要使用工具，如果需要，选择最合适的工具并生成相应的参数。
...
如果不需要使用工具，should_use_tool设为false，并回复：
{
    ...
    "direct_answer": "直接回答用户的内容"
      ...
      if decision.get("should_use_tool") and decision.get("tool_name"):
        ...
        if result:
          # 使用AI格式化结果
          return await self._format_tool_result(question, tool_name, parameters, result)
        else:
