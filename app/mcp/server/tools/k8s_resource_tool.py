...
"""
AI-CloudOps-aiops MCP k8s资源操作工具
Author: Bamboo
Email: bamboocloudops@gmail.com
License: Apache 2.0
Description: k8s资源操作的MCP工具，提供通用资源操作、标签和注解管理功能
"""
...
class K8sResourceTool(K8sBaseTool):
        ...
        return {
            ...
            "properties": {
                "operation": {
                    "type": "string",
                    "description": "要执行的操作",
                    "enum": ["describe_resource", "add_labels", "remove_labels", "add_annotations", "remove_annotations", "apply_yaml", "get_resource_yaml"]
                },
                "config_path": {
                    "type": "string",
                ...
                "resource_type": {
                    ...
                    "enum": ["pod", "service", "deployment", "replicaset", "configmap", "secret", "ingress", "namespace", "node", "pv", "pvc"]
                },
                "resource_name": {
                    "type": "string",
                    "description": "资源名称"
    ...
    async def _describe_resource(self, clients: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
        """描述资源详细信息"""
        try:
    ...
    async def _add_labels(self, clients: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
        """添加标签"""
        try:
            resource_type = parameters.get("resource_type")
            resource_name = parameters.get("resource_name")
    ...
    async def _remove_labels(self, clients: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
        """删除标签"""
        try:
            resource_type = parameters.get("resource_type")
            resource_name = parameters.get("resource_name")
    ...
    async def _add_annotations(self, clients: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
        """添加注解"""
        try:
            resource_type = parameters.get("resource_type")
            resource_name = parameters.get("resource_name")
    ...
    async def _remove_annotations(self, clients: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
        """删除注解"""
        try:
            resource_type = parameters.get("resource_type")
            resource_name = parameters.get("resource_name")
    ...
    async def _apply_yaml(self, clients: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
        """应用YAML配置"""
        try:
            ...
            for resource_def in resources:
                try:
                    ...
                    applied_resources.append({
                        ...
                        "namespace": resource_def.get("metadata", {}).get("namespace")
                    })
                except Exception as e:
                    errors.append({
    ...
    async def _get_resource_yaml(self, clients: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
        """获取资源的YAML表示"""
        try:
            resource_type = parameters.get("resource_type")
            resource_name = parameters.get("resource_name")
            ...
            yaml_content = yaml.dump(resource_dict, default_flow_style=False, allow_unicode=True)
    ...
    async def _get_resource_by_type(self, clients: Dict[str, Any], resource_type: str, name: str, namespace: str, loop) -> Any:
        """根据资源类型获取资源"""
        v1 = clients["v1"]
        ...
        try:
            if resource_type == "pod":
                return await loop.run_in_executor(self._executor, lambda: v1.read_namespaced_pod(name, namespace))
            elif resource_type == "service":
                return await loop.run_in_executor(self._executor, lambda: v1.read_namespaced_service(name, namespace))
            elif resource_type == "deployment":
                return await loop.run_in_executor(self._executor, lambda: apps_v1.read_namespaced_deployment(name, namespace))
            elif resource_type == "replicaset":
                return await loop.run_in_executor(self._executor, lambda: apps_v1.read_namespaced_replica_set(name, namespace))
            elif resource_type == "configmap":
                return await loop.run_in_executor(self._executor, lambda: v1.read_namespaced_config_map(name, namespace))
            elif resource_type == "secret":
                return await loop.run_in_executor(self._executor, lambda: v1.read_namespaced_secret(name, namespace))
            elif resource_type == "ingress":
                return await loop.run_in_executor(self._executor, lambda: networking_v1.read_namespaced_ingress(name, namespace))
            elif resource_type == "namespace":
                return await loop.run_in_executor(self._executor, lambda: v1.read_namespace(name))
            elif resource_type == "node":
                return await loop.run_in_executor(self._executor, lambda: v1.read_node(name))
            elif resource_type == "pv":
                return await loop.run_in_executor(self._executor, lambda: v1.read_persistent_volume(name))
            elif resource_type == "pvc":
                return await loop.run_in_executor(self._executor, lambda: v1.read_namespaced_persistent_volume_claim(name, namespace))
            else:
                return None
    ...
    async def _patch_resource_by_type(self, clients: Dict[str, Any], resource_type: str, name: str, namespace: str, body: Any, loop) -> Any:
        """根据资源类型更新资源"""
        v1 = clients["v1"]
        apps_v1 = clients["apps_v1"]
        networking_v1 = clients["networking_v1"]
        ...
        if resource_type == "pod":
            return await loop.run_in_executor(self._executor, lambda: v1.patch_namespaced_pod(name, namespace, body))
        elif resource_type == "service":
            return await loop.run_in_executor(self._executor, lambda: v1.patch_namespaced_service(name, namespace, body))
        elif resource_type == "deployment":
            return await loop.run_in_executor(self._executor, lambda: apps_v1.patch_namespaced_deployment(name, namespace, body))
        elif resource_type == "replicaset":
            return await loop.run_in_executor(self._executor, lambda: apps_v1.patch_namespaced_replica_set(name, namespace, body))
        elif resource_type == "configmap":
            return await loop.run_in_executor(self._executor, lambda: v1.patch_namespaced_config_map(name, namespace, body))
        elif resource_type == "secret":
            return await loop.run_in_executor(self._executor, lambda: v1.patch_namespaced_secret(name, namespace, body))
        elif resource_type == "ingress":
            return await loop.run_in_executor(self._executor, lambda: networking_v1.patch_namespaced_ingress(name, namespace, body))
        elif resource_type == "namespace":
            return await loop.run_in_executor(self._executor, lambda: v1.patch_namespace(name, body))
        elif resource_type == "node":
            return await loop.run_in_executor(self._executor, lambda: v1.patch_node(name, body))
        else:
    ...
    def _resource_to_dict(self, resource) -> Dict[str, Any]:
        ...
        if hasattr(resource, 'to_dict'):
            ...
            if 'metadata' in resource_dict:
                ...
                resource_dict['metadata'] = {k: v for k, v in metadata.items() if k in keep_fields}
