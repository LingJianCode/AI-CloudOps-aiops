...
class K8sConfigTool(K8sBaseTool):
        ...
        return {
            ...
            "properties": {
                "operation": {
                    "type": "string",
                    "description": "要执行的操作",
                    "enum": ["list_configmaps", "list_secrets", "get_configmap", "get_secret", "create_configmap", "create_secret", "update_configmap", "update_secret", "delete_configmap", "delete_secret"]
                },
                "config_path": {
                    "type": "string",
                ...
                "secret_type": {
                    ...
                    "enum": ["Opaque", "kubernetes.io/service-account-token", "kubernetes.io/dockercfg", "kubernetes.io/dockerconfigjson", "kubernetes.io/basic-auth", "kubernetes.io/ssh-auth", "kubernetes.io/tls"],
                    "default": "Opaque"
                },
                "immutable": {
                    "type": "boolean",
    ...
    async def _list_configmaps(self, v1: client.CoreV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """获取ConfigMap列表"""
        try:
            ...
            if all_namespaces:
                ...
                configmaps = await loop.run_in_executor(
                    self._executor,
                    lambda: v1.list_namespaced_config_map(namespace=namespace, limit=max_results)
                )
    ...
    async def _list_secrets(self, v1: client.CoreV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """获取Secret列表"""
        try:
            namespace = parameters.get("namespace", "default")
            all_namespaces = parameters.get("all_namespaces", False)
            ...
            if all_namespaces:
                ...
                secrets = await loop.run_in_executor(
                    self._executor,
                    lambda: v1.list_namespaced_secret(namespace=namespace, limit=max_results)
                )
    ...
    async def _get_configmap(self, v1: client.CoreV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """获取ConfigMap详细信息"""
        try:
            ...
            configmap = await loop.run_in_executor(
                self._executor,
                lambda: v1.read_namespaced_config_map(name=resource_name, namespace=namespace)
            )
            ...
            configmap_details = {
                ...
                "data_count": len(configmap.data or {}) + len(configmap.binary_data or {})
            }
    ...
    async def _get_secret(self, v1: client.CoreV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """获取Secret详细信息"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
            ...
            secret = await loop.run_in_executor(
                self._executor,
                lambda: v1.read_namespaced_secret(name=resource_name, namespace=namespace)
            )
    ...
    async def _create_configmap(self, v1: client.CoreV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """创建ConfigMap"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
            ...
            created_configmap = await loop.run_in_executor(
                self._executor,
                lambda: v1.create_namespaced_config_map(namespace=namespace, body=configmap_body)
            )
    ...
    async def _create_secret(self, v1: client.CoreV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """创建Secret"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
            ...
            created_secret = await loop.run_in_executor(
                self._executor,
                lambda: v1.create_namespaced_secret(namespace=namespace, body=secret_body)
            )
    ...
    async def _update_configmap(self, v1: client.CoreV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """更新ConfigMap"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
    ...
    async def _update_secret(self, v1: client.CoreV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """更新Secret"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
    ...
    async def _delete_configmap(self, v1: client.CoreV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """删除ConfigMap"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
            ...
            await loop.run_in_executor(
                self._executor,
                lambda: v1.delete_namespaced_config_map(name=resource_name, namespace=namespace)
            )
    ...
    async def _delete_secret(self, v1: client.CoreV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """删除Secret"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
            ...
            await loop.run_in_executor(
                self._executor,
                lambda: v1.delete_namespaced_secret(name=resource_name, namespace=namespace)
            )
