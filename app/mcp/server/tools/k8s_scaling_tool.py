...
class K8sScalingTool(K8sBaseTool):
        ...
        return {
            ...
            "properties": {
                "operation": {
                    "type": "string",
                    "description": "要执行的操作",
                    "enum": ["scale_deployment", "scale_replicaset", "get_hpa_status", "list_hpa", "create_hpa", "delete_hpa"]
                },
                "config_path": {
                    "type": "string",
                ...
                "hpa_config": {
                    ...
                    "properties": {
                        ...
                        "max_replicas": {"type": "integer", "minimum": 1, "maximum": 100, "default": 10},
                        "target_cpu_percent": {"type": "integer", "minimum": 1, "maximum": 100, "default": 80},
                        "target_memory_percent": {"type": "integer", "minimum": 1, "maximum": 100}
                    }
                }
            },
            "required": ["operation"]
    ...
    async def _execute_internal(self, parameters: Dict[str, Any]) -> Any:
        ...
        operation_map = {
            ...
            "get_hpa_status": lambda: self._get_hpa_status(autoscaling_v1, autoscaling_v2, parameters),
            "list_hpa": lambda: self._list_hpa(autoscaling_v1, autoscaling_v2, parameters),
    ...
    async def _scale_deployment(self, apps_v1: client.AppsV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            deployment = await loop.run_in_executor(
                self._executor,
                lambda: apps_v1.read_namespaced_deployment(name=resource_name, namespace=namespace)
            )
    ...
    async def _scale_replicaset(self, apps_v1: client.AppsV1Api, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """伸缩ReplicaSet"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
            ...
            replicaset = await loop.run_in_executor(
                self._executor,
                lambda: apps_v1.read_namespaced_replica_set(name=resource_name, namespace=namespace)
            )
    ...
    async def _list_hpa(self, autoscaling_v1, autoscaling_v2, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """获取HPA列表"""
        try:
            namespace = parameters.get("namespace", "default")
            ...
            if autoscaling_v2:
                if all_namespaces:
                    ...
                    hpas = await loop.run_in_executor(
                        self._executor,
                        lambda: autoscaling_v2.list_namespaced_horizontal_pod_autoscaler(namespace=namespace)
                    )
                is_v2 = True
            else:
                # 使用v1 API
                if all_namespaces:
                    ...
                    hpas = await loop.run_in_executor(
                        self._executor,
                        lambda: autoscaling_v1.list_namespaced_horizontal_pod_autoscaler(namespace=namespace)
                    )
                is_v2 = False
            ...
            for hpa in hpas.items:
                ...
                # 根据API版本添加不同信息
                if is_v2:
                    hpa_info["metrics"] = self._format_v2_metrics(hpa.spec.metrics or [])
                    hpa_info["current_metrics"] = self._format_v2_current_metrics(hpa.status.current_metrics or [])
                else:
                    hpa_info["target_cpu_percent"] = hpa.spec.target_cpu_utilization_percentage
                    hpa_info["current_cpu_percent"] = hpa.status.current_cpu_utilization_percentage
    ...
    async def _get_hpa_status(self, autoscaling_v1, autoscaling_v2, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """获取指定HPA的状态"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
            ...
            # 根据API版本添加不同信息
            if is_v2:
                hpa_status["spec"]["metrics"] = self._format_v2_metrics(hpa.spec.metrics or [])
                hpa_status["status"]["current_metrics"] = self._format_v2_current_metrics(hpa.status.current_metrics or [])
            else:
                hpa_status["spec"]["target_cpu_percent"] = hpa.spec.target_cpu_utilization_percentage
                hpa_status["status"]["current_cpu_percent"] = hpa.status.current_cpu_utilization_percentage
    ...
    async def _create_hpa(self, autoscaling_v1, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """创建HPA"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
    ...
    async def _delete_hpa(self, autoscaling_v1, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """删除HPA"""
        try:
            resource_name = parameters.get("resource_name")
            namespace = parameters.get("namespace", "default")
            ...
            if hasattr(metric, 'resource'):
                formatted_metrics.append({
                    ...
                    "resource": {
                        ...
                        "target": metric.resource.target.to_dict() if metric.resource.target else {}
                    }
                })
        ...
        for metric in metrics:
            if hasattr(metric, 'resource'):
                formatted_metrics.append({
                    ...
                    "resource": {
                        "name": metric.resource.name,
                        "current": metric.resource.current.to_dict() if metric.resource.current else {}
                    }
                })
        return formatted_metrics
