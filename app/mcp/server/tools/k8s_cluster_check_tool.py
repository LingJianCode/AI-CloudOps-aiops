...
"""
AI-CloudOps-aiops MCP k8s集群检查工具
Author: Bamboo
...
class K8sClusterCheckTool(K8sBaseTool):
    ...
    async def _get_cluster_info(self, version_api: client.VersionApi, v1: client.CoreV1Api) -> Dict[str, Any]:
        """获取集群基本信息"""
        try:
            # 使用线程池并行执行同步操作
            ...
            namespaces_task = loop.run_in_executor(self._executor, lambda: v1.list_namespace(limit=50))  # 进一步限制
            
            version_info, namespaces = await asyncio.gather(version_task, namespaces_task, return_exceptions=True)
    ...
    async def _get_node_status(self, v1: client.CoreV1Api) -> Dict[str, Any]:
        ...
        try:
            loop = asyncio.get_event_loop()
            nodes = await loop.run_in_executor(self._executor, lambda: v1.list_node(limit=20))  # 进一步限制节点数量
                    ...
                    if condition.type == "Ready":
                        if condition.status == "True":
                            ...
                            not_ready_nodes.append({
                                ...
                                "message": (condition.message or "")[:100]  # 限制消息长度
                            })
                        break
    ...
    async def _get_recent_events(self, v1: client.CoreV1Api, time_window: int, namespace_filter: Optional[str] = None) -> List[Dict[str, Any]]:
        """获取近期重要事件"""
        try:
            ...
            if namespace_filter:
                events = await loop.run_in_executor(
                    self._executor, 
                    lambda: v1.list_namespaced_event(namespace=namespace_filter, limit=20)  # 减少到20
                )
            else:
                events = await loop.run_in_executor(
                    self._executor, 
            ...
            for event in events.items:
                ...
                if event_time and event_time >= since_time.replace(tzinfo=event_time.tzinfo):
                    if event.type in ["Warning", "Error"] or event.reason in ["Failed", "FailedMount", "FailedScheduling"]:
            ...
            # 按最后发生时间排序
            important_events.sort(key=lambda x: x["last_seen"] or x["first_seen"], reverse=True)
            
            return important_events[:8]  # 返回最近8个事件
        except Exception as e:
            return [{"error": f"获取事件失败: {str(e)}"}]
    
    async def _get_pod_status(self, v1: client.CoreV1Api, namespace_filter: Optional[str] = None) -> Dict[str, Any]:
        """获取Pod状态统计"""
        try:
            loop = asyncio.get_event_loop()
            ...
            if namespace_filter:
                pods = await loop.run_in_executor(
                    self._executor, 
                    lambda: v1.list_namespaced_pod(namespace=namespace_filter, limit=50)  # 减少到50
                )
            else:
                pods = await loop.run_in_executor(
                    self._executor, 
            ...
            for pod in pods.items:
                ...
                if phase not in ["Running", "Succeeded"]:
                    pod_stats["problem_pods"].append({
                        ...
                        "phase": phase,
                        "reason": (pod.status.reason or "Unknown")[:50]  # 限制原因长度
                    })
    ...
    async def _get_error_logs(self, v1: client.CoreV1Api, namespace_filter: Optional[str] = None) -> List[Dict[str, Any]]:
        """获取错误日志（限制数量和时间）"""
        try:
            error_logs = []
            loop = asyncio.get_event_loop()
            ...
            if namespace_filter:
                pods = await loop.run_in_executor(
                    self._executor, 
                    lambda: v1.list_namespaced_pod(namespace=namespace_filter, limit=10)  # 减少到10
                )
            else:
                pods = await loop.run_in_executor(
                    self._executor, 
            ...
            for pod in problem_pods:
                try:
                    ...
                    error_logs.append({
                        ...
                        "sample_log": log_content.strip() if log_content.strip() else "无日志内容"
                    })
    ...
    def _generate_health_report(self, 
        ...
        report_lines.append(f"**检查时间**: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC")
        if namespace_filter:
        ...
        if "error" not in cluster_info:
            report_lines.append(f"- **Kubernetes版本**: {cluster_info['server_version']}")
            report_lines.append(f"- **API版本**: {cluster_info['version']}")
            report_lines.append(f"- **平台**: {cluster_info.get('platform', 'Unknown')}")
            report_lines.append(f"- **命名空间数量**: {cluster_info['namespace_count']}")
            access_status = "✅ 正常" if cluster_info.get('has_cluster_access', False) else "⚠️ 受限"
        ...
        if "error" not in node_status:
            ...
            report_lines.append(f"- **节点健康度**: {health_icon} {health_percentage:.1f}% ({health_status})")
            report_lines.append(f"- **就绪节点**: {node_status['ready']}/{node_status['total']}")
        ...
        if "error" not in pod_status:
            ...
            if total_pods > 0:
                ...
                report_lines.append(f"- **运行中**: {pod_status['running']} ({running_percentage:.1f}%)")
                ...
                if pod_status["problem_pods"]:
                    ...
                    for pod in pod_status["problem_pods"][:5]:
                        report_lines.append(f"- **{pod['namespace']}/{pod['name']}**: {pod['phase']} ({pod['reason']})")
            else:
        ...
        if events and not any("error" in str(event) for event in events):
            ...
            for event in events:
                ...
                report_lines.append(f"- {type_icon} **{event['reason']}** ({event['namespace']}/{event['object']})")
        ...
        warning_events = [e for e in events if isinstance(e, dict) and e.get('type') in ['Warning', 'Error']]
        ...
        report_lines.append(f"- **健康评分**: {status_icon} {health_score}/100 ({status_text})")
