...
class EnhancedRedisVectorStore:
    ...
    async def _parallel_store(self, documents: List[Document], 
    ...
    async def _hybrid_search(
        ...
        semantic_task = asyncio.create_task(
            self._semantic_search(query_vector, k)
        )
        lexical_task = asyncio.create_task(
            self._lexical_search(query, k)
        )
    ...
    async def _semantic_search(
        ...
        """语义搜索"""
        # 使用索引管理器执行向量搜索
        doc_ids_scores = await self.index_manager.search_vectors(
            query_vector, k
        )
...
class IndexManager:
    ...
    async def search_vectors(
        ...
        """搜索向量"""
        if not self.faiss_available or not self.faiss_index or self.faiss_index.ntotal == 0:
            return []
