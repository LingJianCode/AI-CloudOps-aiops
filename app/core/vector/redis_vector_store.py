...
class RedisVectorStore:
    ...
    def __init__(self, 
        ...
        if 'connection_timeout' in self.redis_config and 'socket_connect_timeout' not in self.redis_config:
            self.redis_config['socket_connect_timeout'] = self.redis_config.pop('connection_timeout')
        ...
        for i in range(0, len(documents), batch_size):
            ...
            with self.redis_client.pipeline() as pipe:
                for doc, embedding in zip(batch_docs, batch_embeddings):
                    ...
                    # 存储文档内容
                    pipe.hset(f"{self.doc_key_prefix}{doc_id}", mapping={
    ...
    def _fallback_local_search(self, query: str, k: int, similarity_threshold: float) -> List[Tuple[Document, float]]:
        """降级到本地搜索的备用方案"""
        try:
...
class OptimizedRedisVectorStore(RedisVectorStore):
    ...
    def __init__(self, 
        ...
        """
        super().__init__(redis_config, collection_name, embedding_model, vector_dim, local_storage_path)
    ...
    def add_documents(self, documents: List[Document], batch_size: int = 50) -> List[str]:
        ...
        # 调用父类方法存储到Redis
        doc_ids = super().add_documents(documents, batch_size)
    ...
    def _add_to_faiss_index(self, doc_ids: List[str], embeddings: List[List[float]]):
        ...
        try:
            ...
            # 如果是IVF索引且未训练，先训练
            if (hasattr(self.faiss_index, 'is_trained') and 
                not self.faiss_index.is_trained and 
                len(embeddings) >= 100):  # 需要足够的数据进行训练
                logger.info("开始训练FAISS IVF索引...")
                self.faiss_index.train(vectors)
                logger.info("FAISS IVF索引训练完成")
                
            # 添加向量
            if (not hasattr(self.faiss_index, 'is_trained') or 
            ...
            for doc_id in doc_ids:
                try:
                    ...
                    if word_matches > 0:
                        ...
                        score = min(word_matches / max(content_words, 1) * 5, 1.0)  # 归一化并限制在[0,1]
    ...
    def hybrid_similarity_search(self, 
                                ...
                                similarity_threshold: float = 0.3) -> List[Tuple[Document, float]]:  # 大幅降低阈值
        ...
        try:
            ...
            if self.use_faiss and self.faiss_index is not None and self.faiss_index.ntotal > 0:
                ...
                semantic_results = self.similarity_search(query, k * 3, similarity_threshold * 0.5)  # 进一步降低阈值
            ...
            if semantic_results or lexical_results:
                final_results = self._merge_search_results(
                    ...
                    semantic_weight, lexical_weight, k * 3
                )
                ...
                if len(filtered_results) < k // 2 and final_results:
                    ...
                    logger.debug(f"降低阈值到 {min_threshold} 后获得 {len(filtered_results)} 个结果")
            ...
            with self._lock:
                if self.vector_store is None:
                    ...
                    if embedding_model is None:
                        ...
                        from app.core.agents.assistant import VectorStoreManager as AssistantVSM
                        ...
                        try:
                            ...
                            for frame_info in stack:
                                ...
                                if 'self' in frame.f_locals:
                                    ...
                                    if isinstance(obj, AssistantVSM) and hasattr(obj, 'embedding_model'):
