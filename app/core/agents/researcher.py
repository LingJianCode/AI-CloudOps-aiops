...
class ResearcherAgent:
        ...
        try:
            if config.tavily.api_key:
                self.search_tool = TavilySearchResults(
                    max_results=config.tavily.max_results, api_key=config.tavily.api_key
                )
    ...
    async def _summarize_search_results(self, query: str, results: List[Dict]) -> str:
        ...
        try:
            context = "\n".join(
                [f"标题: {r.get('title', '')}\n内容: {r.get('content', '')}" for r in results[:3]]
            )
...
"""
    ...
    def _clean_error_message(self, error_message: str) -> str:
        ...
        # 移除UUID
        cleaned = re.sub(
            r"[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}", "<uuid>", cleaned
        )
...
"""
    ...
    def get_available_tools(self) -> List[str]:
        """获取可用的研究工具"""
        tools = ["search_kubernetes_solutions", "search_error_solutions"]
