...
class ScalingAdvisor:
    ...
    def _generate_reason(
        ...
        if action == ScalingAction.SCALE_UP:
            if prediction_type == PredictionType.QPS:
                ...
                return f"{type_name}利用率预测值({current_value:.1f}%)过高，建议增加资源"
        elif action == ScalingAction.SCALE_DOWN:
            if prediction_type == PredictionType.QPS:
    ...
    def _calculate_recommendation_confidence(
        ...
        # 基于预测置信度
        base_confidence = prediction.confidence_level if prediction.confidence_level else 0.8
    ...
    def _optimize_recommendations(
        ...
        while i < len(recommendations):
            ...
            while j < len(recommendations):
                ...
                time_diff = (next_rec.trigger_time - current.trigger_time).total_seconds() / 3600
