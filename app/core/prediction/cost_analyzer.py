...
class CostAnalyzer:
    ...
    def _calculate_current_cost(self, constraints: ResourceConstraints) -> float:
        ...
        if constraints:
            ...
            if constraints.min_instances:
                # 假设使用medium实例
                cost += constraints.min_instances * self.default_pricing['instance']['medium']
    ...
    def _calculate_savings_potential(
        ...
        scale_down_count = sum(
            1 for r in scaling_recommendations 
            if r.action.value == "scale_down"
        )
    ...
    def _analyze_cost_trend(
        ...
        # 优化机会
        trend_data['optimization_opportunities'] = self._identify_optimization_opportunities(
    ...
    def _identify_optimization_opportunities(
        ...
        # 检查是否可以使用竞价实例
        scale_down_ratio = sum(1 for r in scaling_recommendations if r.action.value == "scale_down") / max(1, len(scaling_recommendations))
