...
License: Apache 2.0
Description: 模型管理器 - 管理多种预测模型的加载和使用
"""
...
from app.models import PredictionType, ModelInfo
    ...
    def _init_model_paths(self) -> Dict[PredictionType, Dict[str, str]]:
        ...
        for pred_type in PredictionType:
            ...
            if pred_type_key in model_paths_config:
                ...
                result[pred_type] = {
                    'model': os.path.join(base_path, f'{pred_type_key}_prediction_model.pkl'),
                    'scaler': os.path.join(base_path, f'{pred_type_key}_prediction_scaler.pkl'),
                    'metadata': os.path.join(base_path, f'{pred_type_key}_prediction_model_metadata.json')
                }
    ...
    def _validate_model(
        ...
        try:
            ...
            if pred_type == PredictionType.QPS:
                ...
                if pred_value < 0:
                    return False
            elif pred_type in [PredictionType.CPU, PredictionType.MEMORY, PredictionType.DISK]:
    ...
    def _get_default_features(self, pred_type: PredictionType) -> List[str]:
        ...
        if pred_type == PredictionType.QPS:
            return base_features + [
                ...
                'QPS_change', 'QPS_avg_6h'
            ]
        elif pred_type == PredictionType.CPU:
            return base_features + [
                'CPU', 'CPU_1h_ago', 'CPU_1d_ago', 'CPU_1w_ago',
                'CPU_change', 'CPU_avg_6h', 'CPU_std_6h'
            ]
        elif pred_type == PredictionType.MEMORY:
            return base_features + [
                'MEMORY', 'MEMORY_1h_ago', 'MEMORY_1d_ago', 'MEMORY_1w_ago',
                'MEMORY_change', 'MEMORY_avg_6h', 'MEMORY_trend'
            ]
        else:  # DISK
            return base_features + [
                'DISK', 'DISK_1h_ago', 'DISK_1d_ago', 'DISK_1w_ago',
                'DISK_change', 'DISK_avg_24h', 'DISK_growth_rate'
            ]
    ...
    async def get_detailed_info(self) -> Dict[str, Any]:
        ...
        for pred_type, model in self.models.items():
            ...
            model_info = {
                ...
                'has_scaler': pred_type in self.scalers and self.scalers[pred_type] is not None
            }
