...
class UnifiedPredictor:
        ...
        try:
            ...
            for i in range(points):
                ...
                features_aligned = self._align_features_with_model(
                    features, prediction_type, model_data
                )
    ...
    async def _rule_based_prediction(
        ...
        default_growth_rates = {
            PredictionType.QPS: 0.02,      # QPS每小时增长2%
            PredictionType.CPU: 0.01,      # CPU每小时增长1%
            PredictionType.MEMORY: 0.005,  # 内存每小时增长0.5%
            PredictionType.DISK: 0.003     # 磁盘每小时增长0.3%
        }
        # TODO: 可以从配置文件扩展增长率设置
    ...
    def _adjust_prediction_range(self, value: float, prediction_type: PredictionType) -> float:
        """调整预测值到合理范围"""
    ...
    async def is_healthy(self) -> bool:
        """健康检查"""
        return self._initialized and self.model_manager.models_loaded
    ...
    def _get_default_model_features(self, prediction_type: PredictionType) -> List[str]:
        ...
        if prediction_type == PredictionType.QPS:
            return ['QPS'] + base_features + [
                'QPS_1h_ago', 'QPS_1d_ago', 'QPS_1w_ago',
                'QPS_change', 'QPS_avg_6h', 'QPS_std_6h'
            ]
        elif prediction_type == PredictionType.CPU:
            return ['CPU'] + base_features + [
                'CPU_1h_ago', 'CPU_1d_ago', 'CPU_1w_ago',
                'CPU_change', 'CPU_avg_6h', 'CPU_std_6h', 'CPU_max_6h'
            ]
        elif prediction_type == PredictionType.MEMORY:
            return ['MEMORY'] + base_features + [
                'MEMORY_1h_ago', 'MEMORY_1d_ago', 'MEMORY_1w_ago',
                'MEMORY_change', 'MEMORY_avg_6h', 'MEMORY_trend', 'MEMORY_min_6h'
            ]
        else:  # DISK
            return ['DISK'] + base_features + [
                'DISK_1h_ago', 'DISK_1d_ago', 'DISK_1w_ago',
