...
class AppState:
    ...
    def __init__(self):
        ...
        self.max_shutdown_wait = 30  # 最大等待30秒
...
async def lifespan(app: FastAPI):
    ...
    logger.info("主要API端点:")
    logger.info(f"  - GET  {AppConstants.API_VERSION_V1}/predict/health - 预测服务健康检查")
    logger.info(f"  - GET  {AppConstants.API_VERSION_V1}/rca/health     - RCA服务健康检查")
    logger.info(f"  - GET  {AppConstants.API_VERSION_V1}/autofix/health - 自动修复服务健康检查")
    logger.info(f"  - GET  {AppConstants.API_VERSION_V1}/assistant/health - 智能助手健康检查")
    ...
    while app_state.active_requests > 0:
        ...
        if wait_time > app_state.max_shutdown_wait:
            logger.warning(f"等待超时，强制关闭。剩余活跃请求: {app_state.active_requests}")
            break
        ...
        if cleanup_results["success"]:
            ...
            for service_name, result in cleanup_results["services"].items():
                if not result.get("success"):
                    logger.warning(f"服务 {service_name} 清理失败: {result.get('error', '未知错误')}")
            for instance_name, result in cleanup_results["instances"].items():
                if not result.get("success"):
                    logger.warning(f"实例 {instance_name} 清理失败: {result.get('error', '未知错误')}")
...
async def _register_managed_services():
    ...
    try:
        ...
        logger.info(f"服务注册完成，共注册 {len(startup_service._managed_services)} 个服务")
...
async def _register_additional_instances(logger) -> None:
    ...
    try:
        ...
        if assistant_instance:
            startup_service.register_service_instance("enterprise_assistant", assistant_instance)
        ...
        async with self._shutdown_lock:
            try:
                ...
                while app_state.active_requests > 0:
                    wait_time = time.time() - start_wait
                    if wait_time > app_state.max_shutdown_wait:
                        self.logger.warning(f"等待超时，强制关闭。剩余活跃请求: {app_state.active_requests}")
                        break
