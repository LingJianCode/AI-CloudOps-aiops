...
License: Apache 2.0
Description: 预测服务 - 提供负载预测和趋势分析的业务逻辑
"""
...
class PredictionService(BaseService, HealthCheckMixin):
    ...
    async def _do_health_check(self) -> bool:
        ...
        try:
            ...
            return health_result.get("healthy", False) if isinstance(health_result, dict) else bool(health_result)
    ...
    async def predict_trend(
        ...
        # 验证小时数
        if not (ServiceConstants.PREDICTION_MIN_HOURS <= hours <= ServiceConstants.PREDICTION_MAX_HOURS):
        ...
        try:
            ...
            if self._core_service and hasattr(self._core_service, 'model_loader'):
                model_info = await self.execute_with_timeout(
                    lambda: getattr(self._core_service.model_loader, 'get_model_info', lambda: {})(),
                    timeout=30.0,
    ...
    async def get_service_health_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._core_service:
                health_status["model_loaded"] = getattr(self._core_service, 'model_loaded', False)
                health_status["scaler_loaded"] = getattr(self._core_service, 'scaler_loaded', False)
    ...
    def _validate_prediction_params(
        ...
        if current_qps is not None:
            if not (ServiceConstants.PREDICTION_MIN_QPS <= current_qps <= ServiceConstants.PREDICTION_MAX_QPS):
                raise ValidationError(
                    "current_qps",
    ...
    def _wrap_prediction_result(
        ...
        if isinstance(prediction_result, dict):
            ...
            # 否则创建标准格式
            wrapped_result = {
                "instances": prediction_result if isinstance(prediction_result, (int, float)) else 1,
                "timestamp": datetime.now().isoformat()
            }
