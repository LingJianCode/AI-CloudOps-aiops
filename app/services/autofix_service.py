...
License: Apache 2.0
Description: 自动修复服务 - 提供Kubernetes问题诊断和自动修复的业务逻辑
"""
...
from ..common.exceptions import AutoFixError, ValidationError, ResourceNotFoundError
...
class AutoFixService(BaseService):
    ...
    async def fix_kubernetes_deployment(
        ...
        try:
            ...
            deployment_info = await self.execute_with_timeout(
                lambda: self._k8s_fixer.k8s_service.get_deployment(deployment, namespace),
                timeout=ServiceConstants.AUTOFIX_K8S_TIMEOUT,
                operation_name="get_deployment"
            )
            
            if not deployment_info:
                raise ResourceNotFoundError("Deployment", f"{deployment} (namespace: {namespace})")
            ...
            if force_fix and not dry_run:
                ...
                return self._wrap_fix_result(fix_result, deployment, namespace, "supervisor")
            else:
                # 使用K8s修复代理分析和修复
                fix_result = await self.execute_with_timeout(
                ...
                return self._wrap_fix_result(fix_result, deployment, namespace, "k8s_fixer", dry_run)
        ...
        try:
            ...
            if include_events:
                events = await self.execute_with_timeout(
                    lambda: self._k8s_fixer.k8s_service.get_events(namespace, deployment),
                    timeout=ServiceConstants.AUTOFIX_K8S_TIMEOUT,
    ...
    async def get_service_health_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 检查各组件状态
            if self._k8s_fixer:
                health_status["components"]["k8s_fixer"] = ServiceConstants.STATUS_HEALTHY
                ...
                try:
                    ...
                    health_status["components"]["k8s_service"] = ServiceConstants.STATUS_HEALTHY if k8s_health else ServiceConstants.STATUS_UNHEALTHY
                except Exception:
                    health_status["components"]["k8s_service"] = ServiceConstants.STATUS_UNHEALTHY
            else:
                health_status["components"]["k8s_fixer"] = ServiceConstants.STATUS_UNHEALTHY
            
            if self._supervisor:
                health_status["components"]["supervisor"] = ServiceConstants.STATUS_HEALTHY
            else:
                health_status["components"]["supervisor"] = ServiceConstants.STATUS_UNHEALTHY
    ...
    def _validate_k8s_params(self, deployment: str, namespace: str) -> None:
        ...
        if len(deployment) > ServiceConstants.AUTOFIX_MAX_NAME_LENGTH:
            raise ValidationError("deployment", f"部署名称长度不能超过 {ServiceConstants.AUTOFIX_MAX_NAME_LENGTH} 字符")
        
        if not namespace or not isinstance(namespace, str):
            raise ValidationError("namespace", "命名空间不能为空")
        
        if len(namespace) > ServiceConstants.AUTOFIX_MAX_NAME_LENGTH:
            raise ValidationError("namespace", f"命名空间长度不能超过 {ServiceConstants.AUTOFIX_MAX_NAME_LENGTH} 字符")
    ...
    def _wrap_fix_result(
        ...
        if "status" not in wrapped_result:
            wrapped_result["status"] = "completed" if wrapped_result["success"] else "failed"
