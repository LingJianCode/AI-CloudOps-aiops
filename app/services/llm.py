"""
LLM服务集成模块 - AI-CloudOps 系统的大语言模型服务管理中心

本模块实现了一个统一的LLM服务接口，支持多种大语言模型提供商的集成和管理。
系统设计了智能的提供商回退机制，确保在主要模型不可用时能够自动切换到备用模型，
保证服务的高可用性和稳定性。

主要功能：
1. 多提供商支持 - 同时支持OpenAI和Ollama两种主流LLM提供商
2. 智能回退机制 - 主要提供商失效时自动切换到备用提供商
3. 参数验证 - 全面的输入参数验证和错误处理
4. 统一接口 - 为不同提供商提供一致的调用接口
5. 专业化方法 - 针对K8s问题分析、根因分析等场景的专用方法
6. 健康检查 - 实时监控各提供商的服务状态
7. 重试机制 - 内置重试逻辑，提高服务可靠性

支持的LLM提供商：
- OpenAI: GPT-3.5、GPT-4等商业模型
- Ollama: 本地部署的开源模型，如Llama2、Mistral等
