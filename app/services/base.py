...
License: Apache 2.0
Description: 服务基类 - 提供所有业务服务的公共接口和基础功能
"""
    ...
    async def execute_with_timeout(
        ...
        try:
            ...
            raise ServiceUnavailableError(
                service_name=self.service_name,
                details={
                    "error": f"操作 {operation_name} 超时",
                    "timeout": timeout
                }
            )
        except Exception as e:
            self.logger.error(f"服务 {self.service_name} 执行 {operation_name} 失败: {str(e)}")
            raise ServiceUnavailableError(
                service_name=self.service_name,
                details={
                    "error": str(e),
...
class HealthCheckMixin:
    ...
    def create_health_response(
        ...
        response = {
            ...
            "status": ServiceConstants.STATUS_HEALTHY if is_healthy else ServiceConstants.STATUS_UNHEALTHY,
            "healthy": is_healthy,
