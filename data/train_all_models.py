...
class ModelTrainer:
    ...
    def __init__(self, model_type: str):
        ...
        self.metadata_path = (
            f"data/models/{self.model_type}_prediction_model_metadata.json"
        )
    ...
    def extract_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        if "avg_6h" in config["additional_features"]:
            df[f"{main_metric}_avg_6h"] = (
                df[main_metric].rolling(window=6, min_periods=1).mean()
            )
        ...
        if "max_6h" in config["additional_features"]:
            df[f"{main_metric}_max_6h"] = (
                df[main_metric].rolling(window=6, min_periods=1).max()
            )

        if "min_6h" in config["additional_features"]:
            df[f"{main_metric}_min_6h"] = (
                df[main_metric].rolling(window=6, min_periods=1).min()
            )

        if "avg_24h" in config["additional_features"]:
            df[f"{main_metric}_avg_24h"] = (
                df[main_metric].rolling(window=24, min_periods=1).mean()
            )

        if "max_24h" in config["additional_features"]:
            df[f"{main_metric}_max_24h"] = (
                df[main_metric].rolling(window=24, min_periods=1).max()
            )
        ...
        if "trend" in config["additional_features"]:
            ...
            df[f"{main_metric}_trend"] = (
                df[main_metric].rolling(window=12, min_periods=2).apply(safe_trend)
            )
        ...
        for col in numeric_columns:
            if col in df.columns:
                ...
                if inf_count > 0 or nan_count > 0:
                    print(
                        f"警告: 列 {col} 仍有 {inf_count} 个无穷大值和 {nan_count} 个NaN值"
                    )
                    # 强制填充剩余的无效值
        ...
        for name, model in models.items():
            ...
            if isinstance(grid_params, dict) and len(grid_params) == 0:
                ...
                grid_search = GridSearchCV(
                    model, grid_params, cv=tscv, scoring="r2", n_jobs=-1, verbose=0
                )
    ...
    def _get_param_grids(self):
        ...
        if self.model_type == "qps":
            return {
                "Ridge回归": {"alpha": [0.01, 0.1, 1.0, 10.0]},
                "随机森林": {
            ...
            return {
                ...
                "Ridge回归": {"alpha": [0.1, 1.0, 10.0, 100.0]},
            }
    ...
    def visualize(self, X_test, y_test, save_path=None):
        ...
        if hasattr(self.model, "feature_importances_"):
            ...
            ax.set_title(
                f"{self.model_type.upper()} - Feature Importance (Coefficients, Top 10)"
            )
        else:
            ax.text(
                0.5,
...
def generate_synthetic_data(model_type: str, n_samples: int = 8760) -> pd.DataFrame:
    ...
    if model_type == "qps":
        ...
        df["instances"] = np.clip(
            np.round(df["QPS"] / 50 + np.random.normal(0, 0.5, n_samples)), 1, 20
        ).astype(int)
...
def train_all_models():
    ...
    for model_type, performance in results_summary.items():
        ...
        print(
            f"  交叉验证 R²: {performance['cv_mean']:.4f} (+/- {performance['cv_std']:.4f})"
        )
    ...
    with open(summary_file, "w") as f:
        json.dump(
            {"training_date": datetime.now().isoformat(), "models": results_summary},
            f,
            indent=2,
        )
        ...
        try:
            ...
            for case in test_cases:
                ...
                features_array = np.array(
                    [[features_dict.get(feat, 0.0) for feat in feature_names]]
                )
...
def build_features_dict(model_type: str, case: Dict) -> Dict:
    """构建特征字典"""
    metric_map = {"qps": "QPS", "cpu": "CPU", "memory": "MEMORY", "disk": "DISK"}
