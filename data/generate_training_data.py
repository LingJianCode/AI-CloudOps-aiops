    ...
    stats = {
        ...
        'datasets': {
            'qps': {
                ...
                'instances_range': [int(qps_df['instances'].min()), int(qps_df['instances'].max())]
            },
            'cpu': {
                'records': len(cpu_df),
            ...
            'memory': {
                ...
                'memory_range': [float(memory_df['MEMORY'].min()), float(memory_df['MEMORY'].max())],
                'mean_memory': float(memory_df['MEMORY'].mean())
            },
            'disk': {
                'records': len(disk_df),
                'columns': list(disk_df.columns),
                'disk_range': [float(disk_df['DISK'].min()), float(disk_df['DISK'].max())],
...
def visualize_data():
    ...
    ax.plot(range(sample_size), qps_df['QPS'][:sample_size], 'b-', alpha=0.7, label='QPS')
    ax2 = ax.twinx()
    ax2.plot(range(sample_size), qps_df['instances'][:sample_size], 'r-', alpha=0.7, label='Instances')
    ax.set_xlabel('Hours')
