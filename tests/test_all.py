...
def run_all_tests():
    ...
    for i, module in enumerate(test_modules, 1):
        ...
        try:
            ...
            module_duration = time.time() - module_start_time
            logger.error(
                f"💥 {module} 执行异常: {str(e)} (耗时: {module_duration:.2f}秒)"
            )
            results["modules"][module] = {
                "status": "异常",
                "exit_code": -1,
...
def save_test_results(results, logs_dir):
    ...
    try:
        ...
        result_file = (
            logs_dir
            / f"test_all_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        # 保存简化的文本报告
        report_file = (
            logs_dir / f"test_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
