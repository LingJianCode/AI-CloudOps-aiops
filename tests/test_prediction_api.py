            ...
            if result.get("success"):
                ...
                if result.get("data"):
                    ...
                    if isinstance(data, dict):
                        ...
                        if "predicted_data" in data and isinstance(data["predicted_data"], list):
                            print(f"   - 预测点数: {len(data['predicted_data'])}")
                        if "scaling_recommendations" in data:
                            print(f"   - 扩缩容建议: {len(data['scaling_recommendations'])}条")
                        ...
                        if "supported_prediction_types" in data:
                            print(f"   - 支持类型: {data['supported_prediction_types']}")
...
def main():
    ...
    # 2. 测试预测服务健康检查
    test_results.append(
        test_api_endpoint("GET", "/predict/health", test_name="预测服务健康检查")
    )
    ...
    test_results.append(
        test_api_endpoint("POST", "/predict/qps", data=qps_request, test_name="QPS负载预测")
    )
    ...
    test_results.append(
        test_api_endpoint("POST", "/predict/cpu", data=cpu_request, test_name="CPU使用率预测")
    )
    ...
    test_results.append(
        test_api_endpoint("POST", "/predict/memory", data=memory_request, test_name="内存使用率预测")
    )
    ...
    test_results.append(
        test_api_endpoint("POST", "/predict/disk", data=disk_request, test_name="磁盘使用率预测")
    )
