...
class TestPredictionService:
    ...
    @pytest.mark.asyncio
    async def test_qps_prediction_success(self, prediction_service, mock_model_manager, mock_predictor):
        """测试QPS预测成功案例"""
        # Mock 依赖组件
    ...
    @pytest.mark.asyncio
    async def test_cpu_prediction_with_constraints(self, prediction_service, mock_model_manager, 
                                                 mock_predictor, sample_resource_constraints):
        """测试带资源约束的CPU预测"""
        # Mock 依赖组件
        prediction_service._model_manager = mock_model_manager
        prediction_service._predictor = mock_predictor
    ...
    @pytest.mark.asyncio
    async def test_disk_prediction_granularity(self, prediction_service, mock_model_manager, mock_predictor):
        """测试磁盘预测不同时间粒度"""
        # Mock 依赖组件
        prediction_service._model_manager = mock_model_manager
        prediction_service._predictor = mock_predictor
    ...
    @pytest.mark.asyncio
    async def test_anomaly_detection_integration(self, prediction_service, mock_model_manager, mock_predictor):
        """测试异常检测集成"""
        # Mock 依赖组件
        prediction_service._model_manager = mock_model_manager
        prediction_service._predictor = mock_predictor
        ...
        prediction_service._anomaly_detector = AsyncMock()
        prediction_service._anomaly_detector.detect_anomalies.return_value = [mock_anomaly]
        prediction_service._scaling_advisor.generate_recommendations.return_value = []
    ...
    @pytest.mark.asyncio
    async def test_scaling_recommendations_integration(self, prediction_service, mock_model_manager, mock_predictor):
        """测试扩缩容建议集成"""
        # Mock 依赖组件
        prediction_service._model_manager = mock_model_manager
        prediction_service._predictor = mock_predictor
        ...
        # Mock扩缩容建议
        from app.models.predict_models import ScalingRecommendation, ScalingAction
        ...
        prediction_service._scaling_advisor = AsyncMock()
        prediction_service._scaling_advisor.generate_recommendations.return_value = [mock_recommendation]
        prediction_service._anomaly_detector.detect_anomalies.return_value = []
    ...
    @pytest.mark.asyncio
    async def test_pattern_analysis(self, prediction_service, mock_model_manager, mock_predictor):
        """测试模式分析功能"""
        # Mock 依赖组件
        prediction_service._model_manager = mock_model_manager
        prediction_service._predictor = mock_predictor
...
class TestPredictionServiceErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_prediction_execution_error(self, prediction_service, mock_model_manager):
        """测试预测执行错误处理"""
        # Mock预测器抛出异常
        mock_predictor = AsyncMock()
