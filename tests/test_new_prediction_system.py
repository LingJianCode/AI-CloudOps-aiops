...
class PredictionSystemValidator:
    ...
    async def test_service_health(self):
        ...
        try:
            ...
            logger.info(f"支持的预测类型: {health_info.get('supported_prediction_types', [])}")
        ...
        try:
            ...
            if 'models' in model_info:
                for model in model_info['models']:
                    logger.info(f"  - {model.get('type', 'unknown')}: {model.get('name', 'unnamed')}")
    ...
    async def test_cpu_prediction(self):
        ...
        try:
            ...
            logger.info(f"✅ CPU预测成功")
            logger.info(f"  当前CPU: {result['current_value']}%")
            logger.info(f"  预测点数: {len(result['predicted_data'])}")
            logger.info(f"  资源利用率预测数: {len(result.get('resource_utilization', []))}")
            
            # 成本分析
            if result.get('cost_analysis'):
                cost = result['cost_analysis']
                logger.info(f"  当前成本: ${cost.get('current_hourly_cost', 0):.2f}/小时")
                logger.info(f"  预测成本: ${cost.get('predicted_hourly_cost', 0):.2f}/小时")
    ...
    async def test_validation_errors(self):
        ...
        try:
            ...
            validation_tests.append(('invalid_qps', True, f'正确捕获: {str(e)[:50]}...'))
        ...
        try:
            ...
            validation_tests.append(('invalid_cpu', True, f'正确捕获: {str(e)[:50]}...'))
        
        # 测试无效预测时长
        try:
            await self.service.predict_memory_utilization(current_memory_percent=50.0, prediction_hours=0)
            validation_tests.append(('invalid_hours', False, '未捕获无效时长'))
        except Exception as e:
            validation_tests.append(('invalid_hours', True, f'正确捕获: {str(e)[:50]}...'))
    ...
    async def test_configuration_integration(self):
        ...
        try:
            ...
            logger.info(f"QPS扩容阈值: {thresholds.get('qps', {}).get('scale_up', 'N/A')}")
            logger.info(f"CPU扩容阈值: {thresholds.get('cpu', {}).get('scale_up', 'N/A')}")
    ...
    async def generate_report(self):
        ...
        report = {
            ...
            'summary': {
                ...
                'passed_tests': sum(1 for r in self.results.values() if isinstance(r, dict) and r.get('success', False) and 'error' not in r),
                'failed_tests': sum(1 for r in self.results.values() if isinstance(r, dict) and 'error' in r)
            },
        ...
        # 保存报告到文件
        report_file = project_root / 'logs' / f'prediction_test_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
